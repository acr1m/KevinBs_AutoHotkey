...
;~ MATH-INPUT-STYLE SWITCHES FOR EDITOR/BROWSER/ENVIRONMENT COMPATABILITY 
	;~ used for typeSquareRoot() function
	global style_paste := false ;~ default value = false
	global style_backSlash := true ;~ default value = true
	global style_squareRootLeftAmount := 0 ;~ default value = 0
	
;~ SET STYLE FUNCTION
setMathInputStyle(p_1 := false, p_2 := true, p_3 := 3, p_4 := 0){
	style_paste := p_1
	style_backSlash := p_2
	style_squareRootBackspaceAmount := p_3
	style_squareRootLeftAmount := p_4
}
;run the Active Window Sentinel method
activeWindowSentinel()

activeWindowSentinel(){
	while(g_windowSentinelIsOn){
		Sleep, 3000
		if (WinActive("Desmos")){
			setMathInputStyle(false, false, 2, 0)
		} else if (WinActive("Mathway")){
			setMathInputStyle(true, false, 3, 1)
		} else {
			setMathInputStyle(false, true, 3, 0)
		}
	}
}
...
;===============================================================================
	...
	NumpadDiv:: ; square root, 
		Send, {/}
		if (doubleTap()){
			Send, {Backspace %style_squareRootBackspaceAmount%}
			typeSquareRoot()
		}
		return
...
#IfWinActive Mathway 
{
	NumpadMult:: ; superscript, power, multiply, squared, increment, iteration
		Send, {*}
		incrementPowerNoCarrotWithExtraBackspacing()
		return
...
incrementPowerNoCarrotWithExtraBackspacing()
{
	incrementPower(,,,,,false,true)
}

incrementPowerNoCarrot()
{
	incrementPower(,,,,false)
}

incrementPower( p_timeLimit := -1, p_incrementAmount := 1, p_initialBackspaceAmount := 2, p_subsequentBackspaceAmount :=3, p_rightAmount := 1, p_sendCarrot := true, p_extraBackspacing := false){
...
typeSquareRoot(){
	if (style_paste == false){
		if (style_backSlash == false || style_backSlash == 0)
			Send, sqrt{Left %style_squareRootLeftAmount%}
		else if (style_backSlash == true || style_backSlash == 1)
			Send, \sqrt{Left %style_squareRootLeftAmount%}
	} else if (style_paste == true){
		pasteSquareRoot()
	}
}
...
pasteSquareRoot(){
	...
	Send, ^v{Left %style_squareRootLeftAmount%}
	restoreClipboard()
	return
}
